...
class Board:
	...
	def __init__(self):
		...
		self.board_status = [['-' for i in range(9)] for j in range(9)]
		self.block_status = [['-' for i in range(3)] for j in range(3)]
	...
	def find_valid_move_cells(self, old_move):
		...
		allowed_cells = []
		allowed_block = [old_move[0]%3, old_move[1]%3]
		...
		if old_move != (-1,-1) and self.block_status[allowed_block[0]][allowed_block[1]] == '-':
			for i in range(3*allowed_block[0], 3*allowed_block[0]+3):
				for j in range(3*allowed_block[1], 3*allowed_block[1]+3):
					if self.board_status[i][j] == '-':
						allowed_cells.append((i,j))
		else:
			for i in range(9):
				for j in range(9):
					if self.board_status[i][j] == '-' and self.block_status[i/3][j/3] == '-':
						allowed_cells.append((i,j))
	...
	def find_terminal_state(self):
		...
		for i in range(3):						#counts the blocks won by x, o and drawn blocks
			for j in range(3):
				if bs[i][j] == 'x':
					cntx += 1
				if bs[i][j] == 'o':
		...
		if cntx+cnto+cntd <9:		#if all blocks have not yet been won, continue
			return ('CONTINUE', '-')
		elif cntx+cnto+cntd == 9:							#if game is drawn
			return ('NONE', 'DRAW')
	...
	def check_valid_move(self, old_move, new_move):
		...
		if (type(old_move[0]) is not int) or (type(old_move[1]) is not int) or (type(new_move[0]) is not int) or (type(new_move[1]) is not int):
			return False
		if (old_move != (-1,-1)) and (old_move[0] < 0 or old_move[0] > 9 or old_move[1] < 0 or old_move[1] > 9):
			return False
	...
	def update(self, old_move, new_move, ply):
		...
		if (bs[3*x][3*y] == bs[3*x+1][3*y+1] == bs[3*x+2][3*y+2]) and (bs[3*x][3*y] == ply):
			self.block_status[x][y] = ply
			return 'SUCCESSFUL'
		if (bs[3*x+2][3*y] == bs[3*x+2][3*y+1] == bs[3*x+1][3*y+2]) and (bs[3*x+2][3*y] == ply):
			self.block_status[x][y] = ply
			return 'SUCCESSFUL'
